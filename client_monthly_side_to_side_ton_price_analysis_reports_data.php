...
if (isset($_REQUEST['btnRunReport'])) {
	//echo "<pre>"; print_r($_REQUEST); echo "</pre>";
	$fileName = "monthlyReportsPPT_".date('Y-m-d_His').".xls";
	...
	foreach ($client_sel_arr as $client_sel_arr_tmp)	
	{
		...
		foreach($getVendor as $getVendorK => $getVendorV){
			foreach ($period as $dt) {
				...
				if ($client_id == "all") {
					$getQry  = "SELECT water_transaction.invoice_date, sum(weight_in_pound) AS weightval, sum(avg_price_per_pound) AS valueeachval, sum(total_value) AS totalval, vendor_id, water_boxes_report_data.*, water_inventory.*, Month(water_transaction.invoice_date) AS monthNum FROM water_boxes_report_data INNER JOIN water_inventory ON water_boxes_report_data.box_id = water_inventory.ID INNER JOIN water_transaction ON water_boxes_report_data.trans_rec_id = water_transaction.id WHERE water_transaction.vendor_id = " . $getVendorV["id"] . " AND (invoice_date >= '" . $startDate . "' AND invoice_date <= '" . $endDate . " 23:59:59') GROUP BY water_transaction.vendor_id, Month(invoice_date)";
				}else{
					$getQry  = "SELECT water_transaction.invoice_date, sum(weight_in_pound) AS weightval, sum(avg_price_per_pound) AS valueeachval, sum(total_value) AS totalval, vendor_id, water_boxes_report_data.*, water_inventory.*, Month(water_transaction.invoice_date) AS monthNum FROM water_boxes_report_data INNER JOIN water_inventory ON water_boxes_report_data.box_id = water_inventory.ID INNER JOIN water_transaction ON water_boxes_report_data.trans_rec_id = water_transaction.id WHERE water_boxes_report_data.warehouse_id = ".$client_id." AND water_transaction.vendor_id = " . $getVendorV["id"] . " AND (invoice_date >= '" . $startDate . "' AND invoice_date <= '" . $endDate . " 23:59:59') GROUP BY water_transaction.vendor_id, Month(invoice_date)";
				}				
				...
				if ($client_id == "all") {
					$getQry2  = "SELECT invoice_date, total_value AS totalval, vendor_id, water_boxes_report_data.*, water_inventory.* , Month(invoice_date) AS monthNum  FROM water_boxes_report_data INNER JOIN water_inventory ON water_boxes_report_data.box_id = water_inventory.ID INNER JOIN water_transaction ON water_boxes_report_data.trans_rec_id = water_transaction.id WHERE water_transaction.vendor_id = " . $getVendorV["id"] . " AND (invoice_date >= '" . $startDate . "' AND invoice_date <= '" . $endDate . " 23:59:59') ";
				}else{
		...
		foreach ($period as $dt) {
			...
			//echo "excelCol_Weight " . $excelCol_Weight . "<br>";
			...
			//echo "excelCol_Finance " . $excelCol_Finance . "<br>";
			...
			if ($excelCol_Finance == "AZ"){
				$col = -1;
				$excelColS_2_sec = "B";
			}
			if ($excelCol_Finance == "BZ"){
				$col = -1;
				$excelColS_2_sec = "C";
			}
			if ($excelCol_Finance == "CZ"){
				$col = -1;
				$excelColS_2_sec = "D";
			}
			if ($excelCol_Finance == "DZ"){
				$col = -1;
				$excelColS_2_sec = "E";
			}

			$col = $col+1;
			$excelCol_PPT = $excelColS_2_sec . chr(ord($excelColS_2) + ($col) );
			//echo "excelCol_PPT " . $excelCol_PPT . "<br>";
			$object->getActiveSheet()->getStyle($excelCol_PPT.'2')->applyFromArray(array('font'=>array('bold'=>true),'borders'=>array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)),'alignment'=>array('horizontal'=> PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)));
			$object->getActiveSheet()->setCellValue($excelCol_PPT.'2', $monthName."-".$yearNum );
			if ($excelCol_PPT == "AZ"){
				$col = -1;
				$excelColS_2_sec = "B";
			}
		...
		foreach ($period as $dt) {
			...
			$col = $col+1;
			$excelCol_PPT = $excelColS_2_sec . chr(ord($excelColS_3) + ($col) );
			$object->getActiveSheet()->getStyle($excelCol_PPT.'3')->applyFromArray(array('font'=>array('bold'=>true),'borders'=>array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)),'alignment'=>array('horizontal'=> PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)));
			$object->getActiveSheet()->setCellValue($excelCol_PPT.'3', 'Price per Ton' );
			if ($excelCol_PPT == "AZ"){
				$col = -1;
				$excelColS_2_sec = "B";
			}
		...
		while ($rowsVendor = array_shift($getVendor)) {
			...
			foreach ($period as $dt) {
				...
				$total_weight_tons = 0; $net_finamtial_impact = 0;  $price_per_ton = 0;
				...
				if($total_weight_tons > 0 ){
					$price_per_ton = $net_finamtial_impact / $total_weight_tons;
				}else{
					$price_per_ton = 0;
				}
				...
				$col = $col+1;
				$excelCol_PPT = $excelColS_2_sec . chr(ord($excelColS) + ($col) );
				$object->getActiveSheet()->getStyle($excelCol_PPT.$excel_row)->applyFromArray(array ( 'borders' => array( 'bottom' => array( 'style' => PHPExcel_Style_Border::BORDER_THIN ), 'left' => array( 'style' => PHPExcel_Style_Border::BORDER_THIN ), 'right' => array( 'style' => PHPExcel_Style_Border::BORDER_THIN ) ),       'numberformat'=>array('code' => PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00) ) );
				$object->getActiveSheet()->setCellValue($excelCol_PPT.($excel_row), '$'.number_format($price_per_ton,2) );
				if ($excelCol_PPT == "AZ"){
					$col = -1;
					$excelColS_2_sec = "B";
				}
				...
				$arrGrandTotal[$yearNum . $monthName][$rt]['price_per_ton'] = $price_per_ton;
				$rt++;
			}
			$excel_row++;
		}
		...
		foreach ($period as $dt) {
			...
			$colUCB = $colUCB+1;
			$excelCol_PPT = $excelColS_2_sec . chr(ord($excelColUCB) + ($colUCB) );
			$object->getActiveSheet()->getStyle($excelCol_PPT.$excel_row)->applyFromArray(array ( 'borders' => array( 'bottom' => array( 'style' => PHPExcel_Style_Border::BORDER_THIN ), 'left' => array( 'style' => PHPExcel_Style_Border::BORDER_THIN ), 'right' => array( 'style' => PHPExcel_Style_Border::BORDER_THIN ) ),       'numberformat'=>array('code' => PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00) ) );
			$object->getActiveSheet()->setCellValue($excelCol_PPT.($excel_row), '$'.number_format($price_per_ton,2) );
			if ($excelCol_PPT == "AZ"){
				$colUCB = -1;
			...
			$arrGrandTotal1[$yearNum . $monthName]['price_per_ton'] = $price_per_ton;
		}
		...
		foreach ($period as $dt) {
			...
			if($arrGrandTotal[$yearNum . $monthName] != ''){
				$arrTotal[$yearNum . $monthName]['price_per_ton'] = array_sum(array_column($arrGrandTotal[$yearNum . $monthName], 'price_per_ton'));
			}
			...
			$price_per_ton = $arrTotal[$yearNum . $monthName]['price_per_ton'] + $arrGrandTotal1[$yearNum . $monthName]['price_per_ton'];
			...
			$colTotal = $colTotal + 1;
			$excelCol_PPT = $excelColS_2_sec . chr(ord($excelColTotal) + ($colTotal) );
			$object->getActiveSheet()->getStyle($excelCol_PPT.($excel_row+1))->applyFromArray( array ( 'font' => array ( 'bold' => true ), 'fill' => array ( 'type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => array ( 'rgb' => 'A9A9A9' ) ), 'borders' => array( 'bottom' => array( 'style' => PHPExcel_Style_Border::BORDER_THIN ), 'left' => array( 'style' => PHPExcel_Style_Border::BORDER_THIN ), 'right' => array( 'style' => PHPExcel_Style_Border::BORDER_THIN ) ) ) );
			$object->getActiveSheet()->setCellValue($excelCol_PPT.($excel_row+1), '$'.number_format($price_per_ton,2) );
			if ($excelCol_PPT == "AZ"){
				$colTotal = -1;
				$excelColS_2_sec = "B";
			}
	...
	/*HERE ENDS THE EXCEL DATA STORE AND DOWNLOAD*/
