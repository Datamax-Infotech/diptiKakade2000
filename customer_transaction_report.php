		...
		<table border="0" width="100%" cellspacing="0" cellpadding="5">
			...
			<tr>
				<td valign="top">
					<?php
						$sort_order_pre = "ASC";
						//$userInput = filter_var($_GET['sort_order_pre'], FILTER_SANITIZE_STRING);
						...
						<table width="100%" border="0" cellspacing="2" cellpadding="5" bgcolor="#F3F3F3" id="table" class="sortable">
							<thead>
								<tr>
									...
									<th bgcolor="#ABC5DF" class="style12">
									Next<br>Communication&nbsp;
							...
							<?php
								if(isset($_REQUEST['zip'])) 
								{
											...
											elseif(tep_db_num_rows($rs) == 0) {
												...
												while($row = array_shift($rs)) {
													...
													while($objMatchCo = array_shift($res3))
													{
														...
														$summtd_SUMPO = 0; $summtd_SUMPO_p = 0;
													...
													?>
														<tr valign="middle" id="tbl_div<?php echo  $compid?>">
															...
															<td class="tdstyle">
																<input type="text" size="8" name="txt_next_step_dt" id="txt_next_step_dt<?php echo  $compid?>" value="<?php if ($objMatchCo["next_date"]!="") echo date('m/d/Y',strtotime($objMatchCo["next_date"]));?>">
									...
									elseif($_REQUEST["comp_name"]=="asc"){
										...
										array_multisort($vc_array_next_step, SORT_ASC, $MGArray_parent_child_data);//, $vc_array_name, 
									}
										elseif($_REQUEST["next_step"]=="desc"){
										array_multisort($vc_array_next_step, SORT_DESC, $MGArray_parent_child_data);//, $vc_array_name, 
											//print_r($MGArray_parent_child_data);
									}
										//
										elseif($_REQUEST["last_date"]=="asc"){
										array_multisort($vc_array_last_date, SORT_ASC, $MGArray_parent_child_data);//, $vc_array_name, 
									}
										elseif($_REQUEST["last_date"]=="desc"){
										array_multisort($vc_array_last_date, SORT_DESC, $MGArray_parent_child_data);//, $vc_array_name, 
											//print_r($MGArray_parent_child_data);
									}
										//
										elseif($_REQUEST["next_date"]=="asc"){
										array_multisort($vc_array_next_date, SORT_ASC, $MGArray_parent_child_data);//, $vc_array_name, 
									}
										elseif($_REQUEST["next_date"]=="desc"){
										array_multisort($vc_array_next_date, SORT_DESC, $MGArray_parent_child_data);//, $vc_array_name, 
											//print_r($MGArray_parent_child_data);
									}
									//
									...
									foreach ($MGArray_parent_child_data as $MGArraytmp2) { ?>	
										...
										<td class="tdstyle">
											...
											<td class="tdstyle">
												...
												<input type="text" size="8" name="txt_next_step_dt" id="txt_next_step_dt<?php echo  $MGArraytmp2["compid"]?>" value="<?php if ($MGArraytmp2["loopid"] > 0) ?><?php if ($MGArraytmp2["next_date"]!="") echo date('m/d/Y',strtotime($MGArraytmp2["next_date"]));?>">
