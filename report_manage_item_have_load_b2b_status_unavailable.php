...
<head>
	...
	<title>Unavailable Inventory Items with Loads Built Report</title>
...
//Check if  has parent then redirect
...
if ($redirect_mainpg == "yes") {
    ...
    echo "window.location.href=\"report_manage_item_have_load_b2b_status_unavailable.php\";";

    echo "</script>";

    echo "<noscript>";

    echo "<meta http-equiv=\"refresh\" content=\"0;url=report_manage_item_have_load_b2b_status_unavailable.php\" />";
...
<div class="main_data_css">
	...
	<div class="dashboard_heading" style="float: left;">
		...
		<div style="float: left;">
			...
			Unavailable Inventory Items with Loads Built



			<div class="tooltip"><i class="fa fa-info-circle" aria-hidden="true"></i>

			<span class="tooltiptext">This report list inventory that has Loads but B2b Status is unavailable.</span></div><br>
...
$sql = "Select loop_next_load_available_history.*, loop_boxes.b2b_status from loop_next_load_available_history inner join loop_boxes on loop_boxes.id = loop_next_load_available_history.inv_loop_id

	where inactive_delete_flg = 0  and loop_boxes.b2b_status in ('2.0', '2.1', '2.2', '2.3', '2.4', '2.5', '2.6', '2.7', '2.8', '2.9') ORDER BY load_available_date asc";
db();
...
if ($no_of_rec == 0) {?>
		...
		<form name="frm_manage_box_next_load_available_date" id="frm_manage_box_next_load_available_date" action="#">
			...
			<table width="1400px;">
				...
				<tr align="middle" >
					...
					<td colspan="18" style="font-size:14px; font-family: Arial, Helvetica, sans-serif; background-color: #C0CDDA; height: 16px">

					<b>Unavailable Inventory Items with Loads Built</b>
				...
				<tr align="middle" >
					...
					<td bgColor='#ABC5DF' >Item B2B Status

						<a href="<?php echo $sorturl; ?>ASC&sort=b2b_status"><?php echo $imgasc; ?></a>

						<a href="<?php echo $sorturl; ?>DESC&sort=b2b_status"><?php echo $imgdesc; ?></a>
    ...
    while ($so_row_main = array_shift($dt_res_so)) {
        ...
        if ($show_rec == "yes") {
            ...
            $sql_transnotes = "SELECT box_status FROM b2b_box_status where status_key = '" . $so_row_main["b2b_status"] . "'";
            db();
            $result_transnotes = db_query($sql_transnotes);

            while ($myrowsel_transnotes = array_shift($result_transnotes)) {

                $b2bstatus_name = $myrowsel_transnotes["box_status"];
            ...
            $MGarray[] = array('unqid' => $so_row_main["unqid"], 'box_id' => $_REQUEST["id"], 'transid' => $so_row_main["trans_rec_id"], 'load_available_date' => $so_row_main["load_available_date"],
                ...
                'inv_loop_id' => $so_row_main["inv_loop_id"], 'inv_b2b_id' => $so_row_main["inv_b2b_id"], 'inv_box_desc' => $inv_box_desc, 'b2bstatus_name' => $b2bstatus_name,
    ...
    if ($_REQUEST['sort'] == "b2b_status") {

        $MGArraysort_I = array();

        foreach ($MGarray as $MGArraytmp) {

            $MGArraysort_I[] = $MGArraytmp['b2bstatus_name'];
    ...
    foreach ($MGarray as $MGArraytmp2) {
        ...
        ?>
						...
						<tr id="inventory_preord_middle_div_<?php echo $cnt_no; ?>">
							...
							<td width="100px" bgColor="<?php echo $bg; ?>"><?php echo $MGArraytmp2["b2bstatus_name"]; ?></td>
        ...
        $cnt_no += 1;
				...
				<tr align="middle" >
					...
					<td colspan="18" style="font-size: 14px; font-family: Arial, Helvetica, sans-serif; height: 16px"></td>
